 ...
 *
 * The corpus sets out a ten-rung ladder for developing a material issue
 * (sec. 9) and nine rules for how a portal question must be written (sec. 8).
 * Both are here, as data, for the same reason the anomaly list and the claim
 * elements are: a model asked "what should we ask next" will produce good
 * questions and will not produce a method. Asked "which rung of this ladder
 * does this gap need, and write that rung", it is doing the work it is good at.
 *
 * The jargon list is the part with teeth. A question containing the words
 * "meal period", "premium", "regular rate" or "adverse action" does not reach
 * a client, because vet() rejects it and a test asserts vet() rejects it. That
 * is a mechanism. Instructing a model to write simply is a hope.
 *
 * WHAT THE ANSWERS DO
 *
...
export type FollowUpSet = z.infer<typeof FollowUpSet>
  ...
  try {
    response = await client.messages
      .stream({
        ...
        output_config: { effort: 'medium', format: zodOutputFormat(FollowUpSet) },
